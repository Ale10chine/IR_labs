...
#include "nav2_msgs/srv/manage_lifecycle_nodes.hpp" // Service needed to nav initialization
...
#include "nav2_msgs/action/navigate_to_pose.hpp" // Action needed to nav setting of the goal
...
class NavManager : public rclcpp::Node
{
public:
    ...
    // --- CONSTRUCTOR ---
    NavManager() : Node("nav_node")
    {
        // Service client for initialization of the navigation stack
        manage_client_ = create_client<nav2_msgs::srv::ManageLifecycleNodes>("nav_node");
        ...
        timer_ = this->create_wall_timer(
            500ms, 
            std::bind(&NavManager::publish_initial_pose, this)
        );

        // Action client to send the navigation goal
        goal_action_client_ = rclcpp_action::create_client<NavigateToPoseAction>(
            this, "navigate_to_pose");

        RCLCPP_INFO(this->get_logger(), "NavManager is ready");    
    }
    ...
    void send_request()
    {
        ...
        RCLCPP_INFO(get_logger(), "Wait for service");
        manage_client_->wait_for_service();
        
        auto result_future = manage_client_->async_send_request(request);
   ...
   void publish_initial_pose()
    {
        ...
        auto initial_pose_msg = PoseWithCovarianceStamped();
    ...
    void send_goal()
    {
        ...
        goal_action_client_->wait_for_action_server();
        
        auto send_goal_options = rclcpp_action::Client<NavigateToPoseAction>::SendGoalOptions();
        send_goal_options.feedback_callback = 
            std::bind(&NavManager::feedback_callback, this, 
                     std::placeholders::_1, std::placeholders::_2);
        
        goal_action_client_->async_send_goal(goal_msg, send_goal_options);
    }

private:
    rclcpp::Client<nav2_msgs::srv::ManageLifecycleNodes>::SharedPtr manage_client_; // Service client
    ...
    rclcpp_action::Client<NavigateToPoseAction>::SharedPtr goal_action_client_; // Action client
...
int main(int argc, char **argv)
{
    ...
    std::shared_ptr<NavManager> nav_node = std::make_shared<NavManager>();
