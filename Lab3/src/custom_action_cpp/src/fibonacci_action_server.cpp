...
public:
  ...
  : Node("fibonacci_action_server", options)
  {
    ...
    this->action_server_ = rclcpp_action::create_server<Fibonacci>( // templated action type name
...
*/
  ...
  void execute(const std::shared_ptr<GoalHandleFibonacci> goal_handle) {
    ...
    auto feedback = std::make_shared<Fibonacci::Feedback>(); // Feedback
    auto & sequence = feedback->partial_sequence; // Creare an Alias 
    sequence.push_back(0);
    sequence.push_back(1);
    
    auto result = std::make_shared<Fibonacci::Result>(); // Result
    ...
    for (int i = 1; (i < goal->order) && rclcpp::ok(); ++i) {
      ...
      // Update sequence using the fibonacci recurency
